...
class Commercial
{
    ...
    void populate(int a, int b, vector<vector<int>> &initPop, vector<vector<char>> map, vector<vector<int>> &tempVec, int &avWork, int &goods)
    {
        ...
        if(initPop[a][b] == 0 && pLine(a,b,map) == true && avWork >=1 && goods >=1)   
        {
            tempVec[a][b]++;
            avWork -=1;
            goods -=1; 
        }
        else if(initPop[a][b]==0 && adjPop(a,b,initPop,1,1) == true && avWork >=1 && goods >=1)
        {
            tempVec[a][b]++;
            avWork -=1;
            goods -=1;
        }
        else if(initPop[a][b]==1 && adjPop(a,b,initPop,1,2) == true && avWork >=1 && goods >=1)
        {
            tempVec[a][b]++;
            avWork -=1;
            goods -=1; 
